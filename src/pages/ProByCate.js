import { getCateByIDHomePage,getCateByID } from "../api/Cate";
...
const ProByCate = {
   ...
   async before_render({ id }) {
      ...
      const b = await getCateByID(id);
      const a = b;
      const res = await getCateByIDHomePage(a.id);
       this.state.data= res;
      console.log("res" , res);
      setTitle("CATE PAGE")
   },
   render() {
      const {id,name,image} = this.state.data
      return /* html */ `
      <div class="grid grid-flow-row grid-cols-1 md:grid-cols-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-10 mt-10">
         <div class="shadow-lg rounded-lg">
         
         <a href="/product/${id}">
         <img src="${image}" class="rounded-tl-lg rounded-tr-lg" />
         </a>
         <div class="p-5">
         <h3><a href="/product/${name}" class="product_name">${name}</a></h3>                                                                                                                           
         ...
         <div class="flex flex-col xl:flex-row justify-between">
            ...
            <a class="bg-purple-600 rounded-full py-2 px-4 my-2 text-sm text-white hover:bg-purple-700 flex flex-row justify-center" href="/product/${id}">
