...
import { getAllBanner,deleteBannerByID } from "../../../api/banner";
...
const ListBanner = {
   after_render() {
       ...
       delBtn.forEach((element) => { 
         ...
         element.onclick = async () => {
           ...
           if (confirm) {
             await deleteBannerByID(id).then(() => {
   ...
   async cateMapping() {
      let cateHtml = "";
      try {
         const users = await getAllBanner();
         cateHtml = users
            .map((item) => {
               return /* html */ `
               ...
               <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex items-center">
                     ...
                     <a href="/admin/banner/${item.id}/edit"><img class="h-10 w-10 rounded-full" src="${item.image}" alt=""></a>
                     </div>
                     <div class="ml-4">
        ...
        <header class="bg-white shadow">
         <div class="max-w-7x px-4 sm:px-6 lg:px-8 pb-6">
            ...
            <div class="flex-1 min-w-0">
               <h2 class="text-2xl font-bold leading-7 text-gray-900 sm:text-3xl sm:truncate">Danh Sách Banner</h2>
            </div>
            <div class="mt-5 flex lg:mt-0 lg:ml-4">
              
               <a href="/admin/banners/add" class="sm:ml-3">
