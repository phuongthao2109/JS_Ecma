...
import { getAllPost, deletePostByID } from "../../../api/posts";
import toastr from "toastr";
import "toastr/build/toastr.min.css";

const PostList = {
  after_render() {
    const delUserBtn = document.querySelectorAll(".btn-delete-posts");
    delUserBtn.forEach((element) => {
      const id = element.dataset.id;
      element.onclick = async () => {
        const confirm = window.confirm('Are you sure you want to delete?');
        if (confirm) {
          await deletePostByID(id).then(() => {
      ...
      postsHtml = posts
        .map((item) => {
          return /* html */ `
            ...
            <div class="flex items-center">
              <div class="flex-shrink-0 h-10 w-10">
                <a href="/admin/posts/${item.id}/edit"><img class="h-10 w-10 rounded-full" src="${item.image}" alt=""></a>
              </div>
              <div class="ml-4">
                <div class="text-sm font-medium text-gray-900" name="name" id="name">${item.name}</div>
                <div class="text-sm text-gray-500" name="create_by" id="create_by">auth: SUE</div>
              </div>
            </div>
          </td>
            <td class="px-6 py-4 whitespace-nowrap">
            <div class="text-smq text-gray-900 ml-4" name="short_des" id="short_des">${item.short_des}</div>
          ...
          <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
              ...
              <button class="btn-delete-posts bg-red-600 hover:bg-red-700 border border-transparent text-[14px] shadow-sm py-1 px-3 rounded-md text-white" data-id=${item.id}>Delete</button>
          </td>
      </tr>
            `;
                    ...
                    <div class="border overflow-hidden border-b border-gray-200 sm:rounded-lg">
                        <table class="min-w-full divide-y divide-gray-200" id="table-container">
                          ...
                          <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Short Desc</th>
